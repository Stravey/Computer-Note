...
> **多模光纤**
>
> - 可以存在多条不同角度入射的光线在一条光纤中传输。这种光纤就称为**多模光纤**。只适合近距离传输。
...
**5 个网段（Segments）**：一个以太网网络中，最多可包含**5 个独立的物理网段**（如 5 段双绞线或同轴电缆）。

**4 个中继器（Repeaters）**：连接这 5 个网段时，最多可使用**4 个中继器**（或集线器，因集线器本质是多端口中继器）。

**3 个可接设备的网段**：在 5 个网段中，只有**3 个网段可以直接连接计算机、服务器等终端设备**；剩余 2 个网段仅用于连接中继器，不可接终端（仅起信号中继的链路作用）。
